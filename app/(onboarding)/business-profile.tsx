...
export default function BusinessProfileScreen() {
  ...
  const onSubmit = async (values: FormValues) => {
    updateData({
      ...
      businessLogo: businessLogoUri ?? undefined,
    ...
    if (values.businessCategory) formData.append('businessCategory', values.businessCategory);
        ...
        <ScrollView
          ...
          <Animated.View entering={FadeInDown.delay(120)}>
              ...
              render={({ field: { value, onChange } }) => (
                <Field label="Business Category" error={errors.businessCategory?.message}>
                  <StyledInput
                    value={value ?? ''}
                    onChangeText={onChange}
                    placeholder="e.g. Retail, Fashion, Technology, Food"
                  />
                </Field>
              )}
            />
            ...
            <View className="overflow-hidden rounded-2xl border border-canvas-border bg-canvas-surface">
              <TouchableOpacity
                onPress={() => pickImage(setCacDocUri)}
